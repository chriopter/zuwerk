...
export default class extends Controller {
  ...
  async drop(event) {
    ...
    const nest = event.target.closest(".todo-nest-target")
    const parentId = nest ? target.dataset.todoId : target.dataset.parentId
  ...
  async #persist(parentId, position) {
    ...
    const response = await fetch(`/projects/${this.projectIdValue}/todos/${this.dragged.dataset.todoId}/reorder`, {
      method: "PATCH",
