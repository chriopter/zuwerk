...
export default class extends Controller {
  ...
  listDragStart(event) {
    ...
    this.draggedList = event.currentTarget.closest(".task-list-card")
    event.dataTransfer.effectAllowed = "move"
    event.dataTransfer.setData("text/plain", this.draggedList.dataset.listId)
    this.draggedList.classList.add("task-list-row-dragging")
  }
  ...
  dragStart(event) {
    ...
    this.dragged.classList.add("task-list-row-dragging")
  }
  ...
  async drop(event) {
    ...
    if (this.dragged.closest(".task-list-card") === target) return
    await this.#patch(`/projects/${this.projectIdValue}/tasks/${this.dragged.dataset.taskId}/reorder`,
      { task_list_id: target.dataset.listId })
  }
  ...
  dragEnd() {
    this.dragged?.classList.remove("task-list-row-dragging")
    this.draggedList?.classList.remove("task-list-row-dragging")
    this.dragged = null
