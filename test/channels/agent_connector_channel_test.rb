...
class AgentConnectorChannelTest < ActionCable::Channel::TestCase
  ...
  test "registration does not steal an event already claimed by fallback" do
    ...
    event = AgentEvent.create!(recipient: @agent, subject: project.chat.messages.create!(author: human, body: "Fallback"), event_type: "chat_message_mentioned")
  ...
  test "cable-owned lifecycle claims FIFO work and dispatches it exactly once" do
    ...
    first = AgentEvent.create!(recipient: @agent, subject: project.chat.messages.create!(author: human, body: "First"), event_type: "chat_message_mentioned")
    second = AgentEvent.create!(recipient: @agent, subject: project.chat.messages.create!(author: human, body: "Second"), event_type: "chat_message_mentioned")
    publications = []
    dispatcher = ->(event) do
  ...
  test "a replaced lifecycle is fenced immediately before dispatch" do
    ...
    event = AgentEvent.create!(recipient: @agent, subject: project.chat.messages.create!(author: human, body: "Fence"), event_type: "chat_message_mentioned")
    delivered = []
    original_registry = AgentConnectors.registry
  ...
  test "lifecycle releases its database checkout before connector IO" do
    ...
    AgentEvent.create!(recipient: @agent, subject: project.chat.messages.create!(author: human, body: "Checkout"), event_type: "chat_message_mentioned")
    original_registry = AgentConnectors.registry
    registry = AgentConnectors::Registry.new
    AgentConnectors.registry = registry
