...
class AgentTaskWorkTest < ApplicationSystemTestCase
  class ControlledCodexPool
    attr_reader :started

    def initialize(agent:, task:, event:)
      @agent = agent
      @task = task
      @event = event
      @started = Queue.new
    ...
    def prompt(_agent, origin, _prompt, **)
      raise "wrong work origin" unless origin == @task

      @event.acknowledge!
      @started << true
      @continue.pop
      @task.comments.create!(author: @agent, body: "Codex finished the browser task", agent_event: @event)
    end
  ...
  setup do
    ...
    @task = @project.tasks.create!(creator: @human, title: "Verify agent feedback")
  ...
  test "human toggles an emoji and watches real agent work through completion" do
    comment = @task.comments.create!(author: @human, body: "Please verify this")
    visit project_task_path(@project, @task)

    within "#task_comment_#{comment.id}" do
    ...
    pool = ControlledCodexPool.new(agent: @agent, task: @task, event: event)
    ...
    approval = event.agent_approvals.create!(
      request_id: { "wire" => [ "task", event.public_id ] },
      details: { "title" => "Run task checks" },
      options: [ { "optionId" => { "decision" => "allow" }, "name" => "Allow" } ]
    )
    assert_selector "#agent_approval_#{approval.id}", text: "Run task checks"
