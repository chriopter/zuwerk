...
class ProjectApiTest < ActionDispatch::IntegrationTest
  ...
  test "agent lists and views projects" do
    ...
    assert_not response.parsed_body.key?("chat_setting")
  end
  ...
  test "agent uploads an attachment with a message" do
    ...
    post api_project_chat_messages_path(@project), params: { body: "Artifact attached", attachments: [ upload ] }, headers: @headers
  ...
  test "agent sees attachment metadata and can download message attachments" do
    ...
    assert_equal api_project_chat_message_attachment_path(@project, message, attachment), metadata.fetch("download_path")

    get api_project_chat_message_attachment_path(@project, message, attachment), headers: @headers
    assert_response :success
  ...
  test "agent lists and creates messages only through a project" do
    ...
    assert_response :created
    assert_equal @project, ChatMessage.last.project
  ...
  test "event-correlated message creation is idempotent" do
    ...
    assert_difference "ChatMessage.count", 1 do
      post api_project_chat_messages_path(@project), params: { body: "One answer", event_id: event.public_id }, headers: @headers, as: :json
      assert_response :created
    end
    first_id = response.parsed_body.fetch("id")

    assert_no_difference "ChatMessage.count" do
      post api_project_chat_messages_path(@project), params: { body: "Duplicate answer", event_id: event.public_id }, headers: @headers, as: :json
      assert_response :success
    end
    assert_equal first_id, response.parsed_body.fetch("id")
    assert_equal event, ChatMessage.find(first_id).agent_event
  end
  ...
  test "an event can only be published by its recipient in its project" do
    ...
    post api_project_chat_messages_path(@project), params: { body: "Wrong agent", event_id: event.public_id }, headers: { "Authorization" => "Bearer other-token" }, as: :json
    assert_response :not_found
    assert_nil event.reload.publication_chat_message
  end

  test "task assignment events cannot be published as chat messages" do
    human = User.create!(name: "Lin", email: "lin-event@example.com", password: "password1")
    task = @project.tasks.create!(creator: human, title: "Stay in task")
    assignment = task.assignments.create!(agent: @agent, assigned_by: human)
    event = assignment.agent_events.find_by!(recipient: @agent)

    assert_no_difference "ChatMessage.count" do
      post api_project_chat_messages_path(@project), params: { body: "Wrong channel", event_id: event.public_id }, headers: @headers, as: :json
    end
  ...
  test "invalid event-correlated message can be retried and missing events return JSON" do
    ...
    assert_difference "ChatMessage.count", 1 do
      post api_project_chat_messages_path(@project), params: { body: "Valid retry", event_id: event.public_id }, headers: @headers, as: :json
    end
    assert_response :created

    post api_project_chat_messages_path(@project), params: { body: "Missing", event_id: SecureRandom.uuid }, headers: @headers, as: :json
    assert_response :not_found
    assert_equal "AgentEvent not found.", response.parsed_body.fetch("error")
  end
  ...
  test "unscoped message and streaming API routes do not exist" do
    ...
    post "/api/chat_messages/streams", headers: @headers, as: :json
    assert_response :not_found
  end

  test "messages have no legacy streaming state" do
    refute ChatMessage.column_names.include?("state")
    refute ChatMessage.new.respond_to?(:streaming?)
  end

  test "agent lists and creates project tasks with plain text descriptions" do
    task = @project.tasks.create!(creator: @agent, title: "Prepare", description: "<strong>Ready</strong> now")
    @other_project.tasks.create!(creator: @agent, title: "Ignore")

    get api_project_tasks_path(@project), headers: @headers, as: :json

    assert_response :success
    assert_equal [ task.id ], response.parsed_body.map { |item| item.fetch("id") }
    assert_equal "Ready now", response.parsed_body.first.fetch("description")

    assert_difference "Task.count", 1 do
      post api_project_tasks_path(@project), params: { title: "Ship", description: "Deploy safely", status: "open" }, headers: @headers, as: :json
    end
  ...
  test "agent gets and updates a task title description and status" do
    task = @project.tasks.create!(creator: @agent, title: "Prepare", description: "Draft")

    get api_project_task_path(@project, task), headers: @headers, as: :json
    assert_response :success
    assert_equal task.id, response.parsed_body.fetch("id")

    patch api_project_task_path(@project, task), params: { title: "Ship", description: "Final notes", status: "completed" }, headers: @headers, as: :json
  ...
  test "task API create and update normalize positions in both sibling lists" do
    parent = @project.tasks.create!(creator: @agent, title: "Parent", position: 0)
    first = @project.tasks.create!(creator: @agent, title: "First", parent: parent, position: 4)
    second = @project.tasks.create!(creator: @agent, title: "Second", parent: parent, position: 4)
    foreign_root = @other_project.tasks.create!(creator: @agent, title: "Foreign root", position: 7)

    post api_project_tasks_path(@project), params: { title: "Inserted", parent_id: parent.id, position: 1 }, headers: @headers, as: :json
    assert_response :created
    inserted = Task.find(response.parsed_body.fetch("id"))
    assert_equal [ [ first.id, 0 ], [ inserted.id, 1 ], [ second.id, 2 ] ], parent.children.ordered.pluck(:id, :position)

    patch api_project_task_path(@project, inserted), params: { parent_id: "", position: 0 }, headers: @headers, as: :json
    assert_response :success
    assert_equal [ [ inserted.id, 0 ], [ parent.id, 1 ] ], @project.tasks.roots.ordered.pluck(:id, :position)
    assert_equal [ [ first.id, 0 ], [ second.id, 1 ] ], parent.children.ordered.pluck(:id, :position)
    assert_equal [ [ foreign_root.id, 7 ] ], @other_project.tasks.roots.ordered.pluck(:id, :position)
  end

  test "invalid task API move rolls back accompanying updates" do
    parent = @project.tasks.create!(creator: @agent, title: "Parent", position: 0)
    child = @project.tasks.create!(creator: @agent, title: "Child", parent: parent, position: 0)

    patch api_project_task_path(@project, parent), params: { title: "Changed", parent_id: child.id, position: 0 }, headers: @headers, as: :json
  ...
  test "task API rejects cross-project parents and non-integer positions" do
    foreign_parent = @other_project.tasks.create!(creator: @agent, title: "Foreign", position: 8)
    task = @project.tasks.create!(creator: @agent, title: "Local", position: 3)

    patch api_project_task_path(@project, task), params: { parent_id: foreign_parent.id, position: 0 }, headers: @headers, as: :json
    assert_response :unprocessable_entity

    patch api_project_task_path(@project, task), params: { position: "middle" }, headers: @headers, as: :json
    assert_response :unprocessable_entity
    assert_nil task.reload.parent
    assert_equal 3, task.position
    assert_equal 8, foreign_parent.reload.position
  end

  test "task validation errors are JSON and updates cannot change project or creator" do
    task = @project.tasks.create!(creator: @agent, title: "Prepare")

    post api_project_tasks_path(@project), params: { title: "" }, headers: @headers, as: :json
    assert_response :unprocessable_entity
    assert_includes response.parsed_body.fetch("errors"), "Title can't be blank"

    patch api_project_task_path(@project, task), params: { title: "", project_id: @other_project.id, creator_id: -1 }, headers: @headers, as: :json
    assert_response :unprocessable_entity
    assert_equal @project, task.reload.project
    assert_equal @agent, task.creator
  end

  test "task show and update have no unscoped API route" do
    get "/api/tasks/1", headers: @headers, as: :json
    assert_response :not_found

    patch "/api/tasks/1", params: { status: "completed" }, headers: @headers, as: :json
    assert_response :not_found
  end

  test "assigned agent reads full task context and publishes one event-correlated comment" do
    human = User.create!(name: "Ada", email: "task-context@example.com", password: "password1")
    parent = @project.tasks.create!(creator: human, title: "Launch")
    task = @project.tasks.create!(creator: human, title: "Deploy", description: "Use the checklist", parent: parent)
    task.comments.create!(author: human, body: "Production at <strong>14:00</strong>")
    assignment = task.assignments.create!(agent: @agent, assigned_by: human)
    event = assignment.agent_events.find_by!(recipient: @agent)
    ...
    assert_difference "TaskComment.count", 1 do
      post api_project_task_comments_path(@project, task), params: { body: "Deployment complete", event_id: event.public_id }, headers: @headers, as: :json
      assert_response :created
    end

    assert_no_difference "TaskComment.count" do
      post api_project_task_comments_path(@project, task), params: { body: "Duplicate", event_id: event.public_id }, headers: @headers, as: :json
      assert_response :success
    end
    assert_equal event, TaskComment.last.agent_event
  end

  test "mention events and assignments for another task cannot publish task comments" do
    ...
    [ assignment_event, mention_event ].each do |event|
      assert_no_difference "TaskComment.count" do
        post api_project_task_comments_path(@project, task), params: { body: "Wrong context", event_id: event.public_id }, headers: @headers, as: :json
      end
      assert_response :not_found
      assert_equal "AgentEvent not found.", response.parsed_body.fetch("error")
    end
  ...
  test "an agent can publish a correlated response to its task comment mention" do
    human = User.create!(name: "Nora", email: "task-mention@example.com", password: "password1")
    task = @project.tasks.create!(creator: human, title: "Expected task")
    source = task.comments.create!(author: human, body: "@Helper please create the checklist")
    event = source.agent_events.sole

    assert_difference "TaskComment.count", 1 do
      post api_project_task_comments_path(@project, task),
