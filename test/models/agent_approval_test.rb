...
class AgentApprovalTest < ActiveSupport::TestCase
  setup do
    ...
    @event = AgentEvent.create!(recipient: @agent, subject: Message.create!(author: @human, body: "Approval"), event_type: "mentioned")
  ...
  test "accepts false as an exact JSON request ID" do
    @approval.expire!
    event = AgentEvent.create!(recipient: @agent, subject: Message.create!(author: @human, body: "Another approval"), event_type: "mentioned")
    event.transition_to!("running")
